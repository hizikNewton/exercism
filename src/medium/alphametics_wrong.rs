            ...
            for i in 0..result_len {
                ...
                if let Some((nc_rv, nc_opv)) = next_col.split_last() {
                    match nc_opv {
                        ...
                        _ => {
                            if next_col.iter().all(|i| i.is_digit(10))
                                && nc_opv
                                    .iter()
                                    .fold(0, |acc, i| acc + i.to_digit(10).unwrap())
                                    % 10
                                    == nc_rv.to_digit(10).unwrap()
                            {
                                continue;
                            } else {
                                loop {
    ...
    pub fn guess(
        ...
        loop {
            ...
            if let Some(tv) = trial_v {
                ...
                if tv.to_vec().iter().sum::<u32>() == sum_of_opv {
                    if trial_v.unwrap()[0] + 1 == result_value.1 {
                        do_replacement(
                            boxed,
                            vec![
                                (
                                    unknown[0],
                                    &char::from_digit(trial_v.unwrap()[0], 10).unwrap(),
                                ),
                                (
                                    &result_value.0,
                                    &char::from_digit(result_value.1, 10).unwrap(),
                                ),
                            ],
                        );

                        possible_value.remove(trial_v.unwrap());

                        return Ok(());
                    }
                } else if sum_of_opv % 10 == result_value.1 {
                    println!("{:?} rv {:?}", boxed, result_value);
                    if result_value.0.is_digit(10)
                        && result_value.0.to_digit(10).unwrap() == result_value.1
                    {
                        ...
                        todo!("go back to alter")
                    }
                }
            } else {
                possible_value.insert(result_value.1);
                return Err("".to_string());
            }
        }
    }
