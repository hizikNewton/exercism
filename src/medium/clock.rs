...
impl Clock {
    pub fn new(hours: i32, minutes: i32) -> Self { 
        let mut minute_turn_hour = minutes%60;
        let mut my_hours = hours%24 + (minutes/60); 
        loop{
            if my_hours>=24{
                my_hours = my_hours-24;
            }else if my_hours<0 {
                my_hours = 24+(my_hours);
            }
            else if minute_turn_hour<0 {
                my_hours = my_hours- 1;
                minute_turn_hour = 60 + minute_turn_hour;
            }
            else{
                break; 
            }
        ...
        Clock{hours:my_hours,minutes:minute_turn_hour}
    }

    pub fn add_minutes(&self, minutes: i32) -> Self {
        let new_min = self.minutes + minutes;
        Clock::new(self.hours,new_min)
    }
